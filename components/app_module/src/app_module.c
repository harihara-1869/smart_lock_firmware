...
comm_module_config_t AppModule_GetCommConfig(void)
{
     ...
     * generated by AppModule_Init() — key_store_identity_init() has already
     * run by this point. */
...
app_module_err_t AppModule_Init(const comm_module_config_t *comm_cfg)
{
    ...
    /* Load or generate the lock's long-term identity. Must run after
     * storage is warmed (key_store_init / intent_log_init already called
     * by main) and before comm config is fully assembled. */
