...
void app_main(void)
{
    ...
    xTaskCreate((TaskFunction_t)test_integration_run, "test_task", 8192, NULL, 5, NULL);
#endif
    
    /* Keep main task alive so it doesn't clean up memory mapping out from under other tasks */
    vTaskSuspend(NULL);
}
